...
def val_gzsl(test_X, test_label, target_classes, in_package, class_fs, bias=0):
    ...
    with torch.no_grad():
        ...
        for i in range(0, ntest, batch_size):
            ...
            out_package = model(input, class_fs)
...
def val_zs_gzsl(test_X, test_label, unseen_classes, in_package, class_fs, bias=0):
    batch_size = in_package['batch_size']
    model = in_package['model']
    device = in_package['device']
    with torch.no_grad():
...
def eval_zs_gzsl(dataloader, model, device, class_fs, bias_seen, bias_unseen):
    model.eval()
    ...
    with torch.no_grad():
        acc_seen = val_gzsl(test_seen_feature, test_seen_label, seenclasses, in_package, class_fs, bias=bias_seen)
        acc_novel, acc_zs = val_zs_gzsl(test_unseen_feature, test_unseen_label, unseenclasses, in_package,
                                        class_fs, bias=bias_unseen)
...
def visualize_attention(img_ids, alphas_1, alphas_2, S, n_top_attr, attr_name, attr, save_path=None,
                        is_top=True):  # alphas_1: [bir]     alphas_2: [bi]
    n = img_ids.shape[0]
    ...
    for i in range(n):
        ...
        for idx_ctxt, idx_attr in enumerate(idxs_top):
            ...
            ax.set_title(
                "{}\n{}\n{}-{}".format(attr_name[idx_attr], alpha_2[idx_attr], score[idx_attr], attr[idx_attr]),
                {'fontsize': 10})
        #            plt.axis('off')
        fig.tight_layout()
